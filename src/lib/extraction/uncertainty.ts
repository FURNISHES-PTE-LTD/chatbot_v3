...
export function detectUncertainty(message: string): DetectUncertaintyResult {
  ...
  for (const pat of EXPLORATORY_PATTERNS) {
    if (pat.test(lower))
      return { hasUncertainty: true, level: UncertaintyLevel.EXPLORATORY, confidenceAdjustment: -1.0 }
  }
  for (const pat of TENTATIVE_PATTERNS) {
    if (pat.test(lower))
      return { hasUncertainty: true, level: UncertaintyLevel.TENTATIVE, confidenceAdjustment: -0.4 }
  }
  for (const pat of CONDITIONAL_PATTERNS) {
    if (pat.test(lower))
      return { hasUncertainty: true, level: UncertaintyLevel.CONDITIONAL, confidenceAdjustment: -0.3 }
  }
  for (const pat of HEDGED_PATTERNS) {
    if (pat.test(lower))
      return { hasUncertainty: true, level: UncertaintyLevel.HEDGED, confidenceAdjustment: -0.2 }
  }
