    ...
    for (const re of patterns) {
      ...
      while ((m = copy.exec(messageLower)) !== null) {
        ...
        if (term.length > 2) {
          ...
          const c = negType === "retraction" || negType === "over_statement" ? 0.9 : 0.7
  ...
  for (const re of EXCEPTION_PATTERNS) {
    ...
    if (match) {
      ...
      if (groups.length >= 2) {
        ...
        if (negated.length > 1 && exception.length > 1) result.exceptionContext[negated] = exception
      } else if (groups.length === 1 && result.negatedTerms.length > 0) {
        result.exceptionContext[result.negatedTerms[result.negatedTerms.length - 1]] = groups[0].trim()
      }
    }
  }
  return result
...
const STYLE_KEYWORDS = new Set([
  ...
  "bohemian", "contemporary", "vintage", "rustic", "farmhouse",
  "mid-century", "art deco", "japanese", "mediterranean", "coastal",
  "transitional", "glam", "maximalist", "eclectic",
])
const COLOR_KEYWORDS = new Set([
  "blue", "green", "red", "yellow", "purple", "pink", "orange",
  "brown", "gray", "grey", "white", "black", "beige", "cream",
  "navy", "maroon", "teal", "turquoise", "neutral", "warm", "cool",
  "sage", "charcoal", "ivory", "taupe",
])
const FURNITURE_KEYWORDS = new Set([
  "sofa", "couch", "bed", "table", "chair", "desk", "dresser",
  "nightstand", "lamp", "rug", "curtain", "shelf", "cabinet",
])
 ...
 */
export function mapNegatedTermsToFields(negatedTerms: string[]): Record<string, string[]> {
