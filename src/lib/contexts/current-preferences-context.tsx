...
export function CurrentPreferencesProvider({ children }: { children: React.ReactNode }) {
  ...
  const [sourcesByField, setSourcesByField] = useState<Record<string, string | null>>({})

  const refreshPreferences = useCallback(async (conversationId: string) => {
    const prefs = await apiGet<PrefItem[]>(API_ROUTES.conversationPreferences(conversationId))
  ...
  return (
    <CurrentPreferencesContext.Provider
      value={{ preferences, setPreferences, sourcesByField, refreshPreferences }}
    >
      {children}
    </CurrentPreferencesContext.Provider>
  )
