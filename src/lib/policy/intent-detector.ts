...
const FURNITURE_NOUNS = /\b(sofa|chair|table|bed|desk|furniture|sectional|nightstand|dresser)\b/i
...
export function detectIntent(message: string): Intent {
  ...
  if (/\brecommend\b/i.test(lower) && FURNITURE_NOUNS.test(lower)) return "furniture_recs"
  if (/\bsuggest a\b/i.test(lower) && FURNITURE_NOUNS.test(lower)) return "furniture_recs"
  return null
}
