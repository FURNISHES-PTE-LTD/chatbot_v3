 ...
 */
function detectPreferenceRemoval(message: string, currentPrefs: Record<string, string>): ImplicitSignal | null {
  const lower = message.toLowerCase()
  if (!/\b(remove|forget|ignore|don't want|dont want|drop|undo)\b/.test(lower)) return null
  for (const [field, value] of Object.entries(currentPrefs)) {
    if (!value) continue
...
function detectStyleChangeAfterRec(
  ...
  const styleKeywords = ["style", "look", "modern", "scandinavian", "traditional", "minimal", "industrial", "boho", "scandi", "mcm"]
  const hasStyleIntent = styleKeywords.some((k) => message.toLowerCase().includes(k))
  ...
  const assistantMentionedRec = lastTwo.some(
    (m) => m.role === "assistant" && /\b(recommend|suggest|option|choice)\b/i.test(m.content)
  )
  if (lastWasAssistant && assistantMentionedRec) {
    return {
      type: "style_change_after_rec",
      field: "style",
      comment: "User changed or clarified style shortly after a recommendation; recommendation may have missed the mark.",
    }
  }
  return null
}
