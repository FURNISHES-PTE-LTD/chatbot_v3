...
function isValidPlacement(
  ...
  for (const ob of obstacles) {
    ...
    const posEnd = pos.offsetInches + (pos.wall === "north" || pos.wall === "south" ? widthInches : depthInches)
    if (pos.wall === ob.wall && !(posEnd <= obStart || posStart >= obEnd)) return false
  }
  const margin = WALKWAY
  if (pos.wall === "north" || pos.wall === "south") {
    if (pos.offsetInches < margin || pos.offsetInches + widthInches > (input.roomWidthInches - margin)) return false
  } else {
    if (pos.offsetInches < margin || pos.offsetInches + depthInches > (input.roomLengthInches - margin)) return false
  }
  return true
}
 ...
 */
function scorePlacement(placement: FurniturePlacement, input: LayoutInput): number {
  let score = 50
...
export function planLayout(input: LayoutInput): LayoutOption[] {
  ...
  const { roomWidthInches, roomLengthInches, doors, windows } = input
  ...
  const walls: Array<"north" | "south" | "east" | "west"> = ["north", "south", "east", "west"]
  const bedPlacements: FurniturePlacement[] = []

  for (const wall of walls) {
    const maxOffset = (wall === "north" || wall === "south" ? roomWidthInches : roomLengthInches) - bedLength
    for (let offset = BED_CLEARANCE.sides; offset <= maxOffset - BED_CLEARANCE.sides; offset += 12) {
      const pos: Position = { wall, offsetInches: offset, widthInches: bedLength, depthInches: bedWidth }
  ...
  for (let i = 0; i < Math.min(3, bedPlacements.length); i++) {
    ...
    for (const wall of walls) {
      const len = wall === "north" || wall === "south" ? roomWidthInches : roomLengthInches
      for (let offset = WALKWAY; offset <= len - deskWidth - WALKWAY; offset += 12) {
        const pos: Position = { wall, offsetInches: offset, widthInches: deskWidth, depthInches: deskDepth }
    ...
    const bestDesk = deskCandidates.length
      ? deskCandidates.sort((a, b) => scorePlacement(b, input) - scorePlacement(a, input))[0]
      : null
    const placements = [bed, ...(bestDesk ? [bestDesk] : [])]
    const score = placements.reduce((s, p) => s + scorePlacement(p, input), 0) / placements.length
    options.push({
      placements,
 ...
 */
export function parseRoomDimensions(text: string): { widthInches: number; lengthInches: number } | null {
  ...
  const byMatch = lower.match(/(\d+)\s*(ft|feet|')\s*by\s*(\d+)\s*(ft|feet|')?/)
  if (byMatch) {
