...
const BED_KEYWORDS = ["bed", "bedroom", "side of bed", "foot of bed", "around the bed"]
const DESK_KEYWORDS = ["desk", "desk depth", "desk size", "laptop desk", "desktop desk", "drafting"]
const RUG_KEYWORDS = [
  "rug",
  "rug size",
  "rug sizing",
...
export function lookupDesignRule(query: string): string | null {
  ...
  if (RUG_KEYWORDS.some((k) => lower.includes(k))) {
    ...
    lines.push(`Example: queen bed bedroom rug typically ${queenRug.width}" x ${queenRug.length}".`)
  }
