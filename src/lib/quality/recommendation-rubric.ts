...
import { z } from "zod"
import { getOpenAIKey, OPENAI_PRIMARY_MODEL } from "@/lib/core/openai"
...
/**
 * Grade a single recommendation text against the rubric. If it fails, return a revised version.
 */
...
/**
 * Grade an item's why_it_fits and optionally revise it.
 */
export async function gradeRecommendationItem(
  item: { name: string; category: string; why_it_fits: string; estimated_price?: number | null },
  preferences: Record<string, string>
