...
const COMPARATIVE_REFERENCES: Record<string, OutcomeAttributes> = {
  "boutique hotel": {
    ...
    furnitureLayout: "curated pieces, statement lighting, plush seating, upscale feel",
  },
  "restoration hardware": {
    designStyle: ["transitional", "industrial"],
    colorTheme: ["neutral", "warm"],
    furnitureLayout: "substantial furniture, natural materials, industrial accents",
  },
  "coffee shop": {
 ...
 */
export function detectOutcomeStatements(message: string): Array<[string, number]> {
 ...
 */
export function inferAttributesFromOutcome(outcome: string): OutcomeAttributes | null {
  const outcomeLower = outcome.toLowerCase().trim()
  if (OUTCOME_TO_ATTRIBUTES[outcomeLower]) return OUTCOME_TO_ATTRIBUTES[outcomeLower]
 ...
 */
export function extractIndirectPreferences(message: string): Record<string, string | string[]> {
  const statements = detectOutcomeStatements(message)
  const combined: { designStyle: string[]; colorTheme: string[]; furnitureLayout: string[] } = {
    designStyle: [],
    colorTheme: [],
    furnitureLayout: [],
  }
  for (const [outcome] of statements) {
    ...
    if (inferred) {
      if (inferred.designStyle) combined.designStyle.push(...inferred.designStyle)
      if (inferred.colorTheme) combined.colorTheme.push(...inferred.colorTheme)
      if (inferred.furnitureLayout) combined.furnitureLayout.push(inferred.furnitureLayout)
    }
  }
  ...
  if (combined.furnitureLayout.length) out.furniture = combined.furnitureLayout.join("; ")
  return out
}
