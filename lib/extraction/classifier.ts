  ...
  for (const pat of CLARIFICATION_PATTERNS) {
    if (pat.test(lower)) {
      if (_chatHistory?.length) {
        const lastAssistant = [..._chatHistory].reverse().find((m) => m.role === "assistant")
        const content = (lastAssistant?.content ?? "").toLowerCase()
        if (content.includes("clarification") || content.includes("?"))
          return { intent: MessageIntent.CLARIFICATION_RESPONSE, confidence: 0.9 }
      }
    }
  }

  for (const pat of RETRACTION_PATTERNS)
    if (pat.test(lower)) return { intent: MessageIntent.RETRACTION, confidence: 0.9 }
  for (const pat of UPDATE_PATTERNS)
    if (pat.test(lower)) return { intent: MessageIntent.UPDATE, confidence: 0.85 }
  for (const pat of NEGATION_PATTERNS)
    if (pat.test(lower)) return { intent: MessageIntent.NEGATION, confidence: 0.8 }
  for (const pat of UNCERTAIN_PATTERNS)
    if (pat.test(lower)) return { intent: MessageIntent.UNCERTAIN, confidence: 0.75 }
  for (const pat of EXPLORATORY_PATTERNS)
    if (pat.test(lower)) return { intent: MessageIntent.EXPLORATORY, confidence: 0.9 }

  const isQuestion = QUESTION_PATTERNS.some((p) => p.test(lower))
  if (isQuestion) {
    const hasPref = PREFERENCE_KEYWORDS.some((k) => lower.includes(k))
    if (hasPref) return { intent: MessageIntent.DIRECT_PREFERENCE, confidence: 0.7 }
    return { intent: MessageIntent.QUESTION, confidence: 0.8 }
  }

  for (const pat of INDIRECT_PATTERNS)
    if (pat.test(lower)) return { intent: MessageIntent.INDIRECT_PREFERENCE, confidence: 0.75 }

  const hasKeywords = PREFERENCE_KEYWORDS.some((k) => lower.includes(k))
  if (hasKeywords) return { intent: MessageIntent.DIRECT_PREFERENCE, confidence: 0.7 }
  if (lower.split(/\s+/).length <= 2) return { intent: MessageIntent.EXPLORATORY, confidence: 0.6 }
