...
export function validateSpans(
  ...
  for (const span of spans) {
    ...
    const e = Number(end)
    if (Number.isNaN(s) || Number.isNaN(e) || s < 0 || e > message.length || s >= e)
      continue
...
export function getEvidenceForField(
  message: string,
  evidenceList: Array<{ field?: string; start?: number; end?: number; text?: string }>,
  field: string
): EvidenceSpan[] {
  if (!evidenceList?.length) return []
  const fieldSpans = evidenceList
    .filter((s) => s && s.field === field && s.start != null && s.end != null && s.text != null)
