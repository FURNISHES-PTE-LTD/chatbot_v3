...
const RETRACTION_PATTERNS: Array<{ pattern: RegExp; group: number }> = [
  { pattern: /\b(?:actually|wait|hold on|never mind|forget)\s+(?:about\s+)?(?:what\s+i\s+said\s+about\s+)?([a-z\s]+?)(?:\s|$|,|\.)/, group: 1 },
  { pattern: /\b(?:changed|change)\s+my\s+mind\s+(?:about\s+)?([a-z\s]+?)(?:\s|$|,|\.)/, group: 1 },
  { pattern: /\b(?:scratch|ignore|disregard)\s+(?:that|what i said about)\s+([a-z\s]+?)(?:\s|$|,|\.)/, group: 1 },
]

const UPDATE_PATTERNS: Array<{ pattern: RegExp; oldGroup: number; newGroup: number }> = [
  { pattern: /\b(?:actually|wait|correction)\s+([a-z\s]+?)(?:\s|$|,|\.)/, oldGroup: 1, newGroup: 0 },
  { pattern: /\b(?:changed|change)\s+(?:from|it to)\s+([a-z\s]+?)\s+(?:to|from)\s+([a-z\s]+?)(?:\s|$|,|\.)/, oldGroup: 1, newGroup: 2 },
  { pattern: /\b(?:not|no longer)\s+([a-z\s]+?)\s+(?:anymore|now|,)\s+(?:but|instead)\s+([a-z\s]+?)(?:\s|$|,|\.)/, oldGroup: 1, newGroup: 2 },
]
...
export type MatchingPreference = { field: string; value: string; isList: boolean }
...
export function findMatchingPreference(
  ...
  for (const field of fieldIds) {
    ...
    if (isList) {
      ...
      if (raw.toLowerCase().includes(search) || search.includes(raw.toLowerCase()))
...
export function createStateChangeUpdate(
  ...
  if (intent.changeType === "retraction") {
    for (const oldVal of intent.oldValues) {
      const match = findMatchingPreference(oldVal, currentPrefs, recentMessageContents)
      if (match && match.field !== "unknown") {
        removals.push({ field: match.field, value: match.value })
        if (match.isList) {
          const raw = currentPrefs[match.field]
          const items = raw.split(",").map((s) => s.trim()).filter((s) => s.toLowerCase() !== match.value.toLowerCase())
      ...
      if (match && match.field !== "unknown") {
        if (match.isList) {
          ...
          const newItems = items.map((item) =>
            item.toLowerCase() === match.value.toLowerCase() ? intent.newValue! : item
          )
